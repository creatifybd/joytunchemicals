...
export default function SiteLayout(){
  ...
  const style={'--ink':a.ink,'--green':a.green,'--lime':a.lime,'--paper':a.paper,'--heading-font':`'${a.headingFont} Variable',sans-serif`,'--body-font':`'${a.bodyFont} Variable',sans-serif`,'--corner':`${a.radius}px`}
  return <MotionConfig reducedMotion={a.motion?'user':'always'}><ToastProvider><div className={`public-site ${a.motion?'':'motion-off'}`} style={style}>
    ...
    <nav className="desktop-nav" aria-label="Main navigation">{site.navigation.map(n=><NavLink key={n.url} to={safeUrl(n.url)} end={n.url==='/'}>{n.label}</NavLink>)}<button className="nav-category-toggle" aria-label="Show care categories" aria-expanded={categoriesOpen} onClick={()=>setCategoriesOpen(v=>!v)}><ChevronDown size={16}/></button></nav>
