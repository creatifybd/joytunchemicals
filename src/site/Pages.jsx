...
export function HomePage(){
  ...
  const content = <>
    ...
    {h.showCategories&&<section className="section-space shell" id="care-collections"><Reveal className="section-heading"><div><p className="eyebrow">A PLACE FOR EVERY KIND OF CARE</p><EditorialTitle text={h.collectionsTitle}/></div><p>{h.collectionsText}</p></Reveal>
      <div className="category-grid">{careCategories.map((c,i)=><Reveal key={c.id} delay={i%3*0.06}><Link to={`/products?category=${c.id}`} className={`category-card category-${c.id}`} style={{backgroundColor:c.color,'--category-color':c.color}}><div className="category-top"><span>0{i+1} / {c.name}</span><span className="category-arrow"><ArrowUpRight size={22}/></span></div><ManagedImage reference={c.image} products={products} alt={`${c.name} collection`} loading="lazy" decoding="async"/><div className="category-caption"><h3>{c.title}</h3><p>{c.description}</p></div></Link></Reveal>)}</div>
    </section>}
    {h.showFeatured&&<section className="featured-section section-space"><div className="shell"><Reveal className="section-heading"><div><p className="eyebrow">MEET YOUR EVERYDAY ESSENTIALS</p><EditorialTitle text={h.featuredTitle}/></div><Link className="text-link" to="/products">Explore the collection <ArrowUpRight size={19}/></Link></Reveal><div className="product-grid featured-grid">{featured.map((p,i)=><Reveal key={p.id} delay={i*0.05}><ProductCard product={p}/></Reveal>)}</div><CollectionShowcase/></div></section>}
...
export function ProductsPage(){
  ...
  return <>
    <section className="catalogue-intro shell"><p className="eyebrow">{site.products.eyebrow}</p><div><EditorialTitle text={site.products.title} as="h1"/><p>{site.products.description}</p></div></section>
...
export function AboutPage(){
  ...
  const brandNames=[...new Set(products.filter(p=>p.status==='active').map(p=>p.brand).filter(Boolean))]
  return <>
    <section className="story-intro shell"><p className="eyebrow">{a.eyebrow}</p><EditorialTitle text={a.title} as="h1"/><div className="story-lede"><span className="story-symbol"><Flower2 size={58} strokeWidth={1}/></span><p>{a.intro}</p></div></section>
    <section className="story-visual shell">{site.media.showLifestyleImages&&<img className="story-lifestyle-image" src={safeUrl(site.media.aboutImage)} alt={site.media.aboutAlt} loading="lazy"/>}<div className="story-image-composition"><span aria-hidden="true">{a.artTitle}</span>{a.images.slice(0,4).map((slug,i)=><ManagedImage key={slug} className={`story-product-${i}`} reference={slug} products={products} alt={slug.replaceAll('-',' ')} loading="lazy"/>)}</div><div className="story-caption"><p>THE JOYTUN FAMILY</p><p>Home care. Personal care. Everyday care.</p></div></section>
    <section className="story-details shell section-space"><Reveal><p className="eyebrow">OUR STORY</p><EditorialTitle text={a.storyTitle}/></Reveal><Reveal><p className="large-paragraph">{a.lead}</p><p>{a.paragraph1}</p><p>{a.paragraph2}</p><ActionLink to="/products">Meet our products</ActionLink></Reveal></section>
    {a.showBrands&&<section className="brand-family"><div className="shell"><p className="eyebrow">OUR BRANDS. ONE FAMILY.</p><EditorialTitle text={a.brandsTitle}/><div className="brand-family-grid">{brandNames.map((brand,i)=><Link to={`/products?q=${encodeURIComponent(brand)}`} key={brand}><small>0{i+1}</small><span>{brand}</span><ArrowUpRight size={24}/></Link>)}</div></div></section>}
    {a.showPurpose&&<section className="purpose-section shell section-space"><Reveal><p className="eyebrow">WHAT WE’RE HERE FOR</p><EditorialTitle text={a.purposeTitle}/></Reveal><div className="purpose-grid">{site.values.map((v,i)=><Reveal key={i}><span>{String(i+1).padStart(2,'0')}</span><h3>{v.title}</h3><p>{v.text}</p></Reveal>)}</div></section>}
    <PartnerSection/>
  </>
}
...
export function ContactPage(){
  ...
  return <>
    <section className="contact-intro shell"><div className="contact-title-copy"><p className="eyebrow">{c.eyebrow}</p><EditorialTitle text={c.title} as="h1"/><p>{c.description}</p></div>{site.media.showLifestyleImages&&<img className="contact-lifestyle-image" src={safeUrl(site.media.contactImage)} alt={site.media.contactAlt} loading="eager"/>}</section>
